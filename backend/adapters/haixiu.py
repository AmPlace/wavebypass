...
import httpx
from streamget import HaixiuLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_haixiu(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_haixiu_room_id", "嗨秀 房间号不能为空")

    url = f"https://www.haixiutv.com/{room_id}"

    try:
        live = HaixiuLiveStream(cookies="")
