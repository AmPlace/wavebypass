...
import httpx
from streamget import LiveMeLiveStream

from . import ADAPTER_SUCCESS_TTL_SECONDS, AdapterRequest, AdapterResolveError


async def resolve_liveme(request: AdapterRequest, client: httpx.AsyncClient) -> dict[str, Any]:
    room_id = request.resource_id.strip("/")
    if not room_id:
        raise AdapterResolveError("invalid_liveme_room_id", "LiveMe 房间号不能为空")

    url = f"https://www.liveme.com/{room_id}/index.html"

    try:
        live = LiveMeLiveStream(cookies="")
