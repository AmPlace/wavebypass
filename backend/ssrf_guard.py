...
_HARD_BLOCKED_NETWORKS = (
    ...
    ipaddress.ip_network("fc00::/7"),          # IPv6 ULA，含 GCP 元数据 fd00:ec2::254
...
async def assert_safe_target_url(url: str, *, allow_private: bool | None = None) -> None:
    ...
    if parsed.scheme.lower() not in {"http", "https"}:
        raise UnsafeTargetError("只允许 http/https 地址")
    host = parsed.hostname
    if not host:
        raise UnsafeTargetError("缺少 hostname")

    effective_allow_private = ALLOW_PRIVATE if allow_private is None else bool(allow_private)
...
def assert_safe_host_ips(host: str, *, allow_private: bool | None = None) -> None:
