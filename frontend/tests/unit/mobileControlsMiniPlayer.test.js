...
test('Mobile header controls keep 44px hit targets but reduce the visible circle and icon', () => {
  const css = source('src/style.css')
  assert.match(css, /\.mobile-action-btn::before \{[\s\S]*?inset: 4px;/)
  assert.match(css, /\.mobile-action-btn > svg \{[\s\S]*?width: 1\.0625rem;[\s\S]*?height: 1\.0625rem;/)
  assert.match(css, /\.mobile-search-collapsed::before \{[\s\S]*?inset: 4px;/)
...
test('IPTV filter chips keep 44px interaction boxes with 36px visual pills', () => {
  ...
  assert.match(css, /\.iptv-main \.tag-filter-row__item::before,[\s\S]*?inset: 4px;/)
})
...
test('Mobile BottomPlayer uses bounded 72px presentation and preserves playback controls', () => {
  ...
  assert.match(css, /\.mobile-player-list-btn::before \{[\s\S]*?inset: 4px;/)
