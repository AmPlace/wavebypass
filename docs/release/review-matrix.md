...
| 27 | Provider | Radio playback and browser transport | `routers/media_proxy.py`, `radio_core.py`, `main.py` audio MIME path | `radioAudioEngine.js`, `AudioEngine.vue`, `BottomPlayer.vue` | Signed radio handles and persisted Radio source descriptors | `architecture/radio.md`, `architecture/raw-streams.md` | `test_radio_playback_transport.py`, `radioAudioEngineAttempt.test.js`, `audioEngineComponent.test.js`, `radioStore.test.js` | NOT FULLY REVIEWED | `REV-RADIO-002` | P1; Radio intent/attempt continuity and play-error classification are focused-tested in `197cc679`; run the remaining Yunting HLS/MyRadio audio HTTP/provider/browser matrix |
...
| REV-RADIO-002 | Radio playback | P1 | SHOULD FIX BEFORE RELEASE for Radio release | Commit `197cc679` adds focused evidence for intent propagation, async resolve attempt continuity, explicit retry, fallback play reset, and `play()` error classification; static browser click and pause/Play checks passed. Full provider/browser playback acceptance remains open, and dynamic Yunting/MyRadio acceptance was not run because the isolated runtime exposed no visible dynamic stations. | Test Yunting HLS, MyRadio audio HTTP, source switch, explicit play, and failure mapping in real browsers. |
